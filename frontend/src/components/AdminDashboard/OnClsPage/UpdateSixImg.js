...
import {getSingleSixImg, updateSixImg, clearErrors } from '../../../actions/siximgAction'
import {UPDATE_SIXIMG_RESET} from '../../../constants/siximgConstant'



const UpdateSixImg = ({ match, history }) => {
    const dispatch = useDispatch();
    const alert = useAlert()  
  
    const { loading:sixLoading,error, content } = useSelector((state) => state.singleSiximg);

    const {
      loading,
        error: updateError,
        isUpdated,
      } = useSelector((state) => state.updateSiximg);
        ...
        useEffect(() => {
            if(content && content._id !== contentId) {
              dispatch(getSingleSixImg(contentId))
            } else{
            ...
            if (isUpdated) {
              ...
              dispatch({ type: UPDATE_SIXIMG_RESET });
            }
            ...
            const updateFormSubmit = (e) => {
                ...
                dispatch(updateSixImg(contentId, myFormData))
              };
